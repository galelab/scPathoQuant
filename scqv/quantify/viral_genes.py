...
def quantify_reads(output_path,filename, virus_names):
    ...
    for k, t in reads_mapping.items():
        if len(t.keys()) > 0:
            ...
            if df2.empty:
                print("STATUS: no reads able to be quantified for viral genes for virus "+k+" will not add genes to 10x filtered files")
                dfumiall[k]=False
            else:
                ...
                df_umi.to_csv(os.path.join(output_path, "virus_al_gene_counts_"+k+".csv"))
                dfumiall[k]=  df_umi
...
def htseq_run(args, virus_names):
    ...
    if len(files_gtf) > 1:
        ...
        dfumi = quantify_reads(args.output_path, os.path.join(args.output_path, "virus_genes_al_sort_counts.sam"), virus_names)
        return dfumi
